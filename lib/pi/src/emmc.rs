...
//! Driver - BCM2711 EMMC2 controller
//!
//! adapted from https://github.com/nihalpasham/rustBoot by Nihal Pasham
#![allow(warnings)]
...
fn timer_wait_micro(delay: u64) {
    timer::spin_sleep(Duration::from_micros(delay));
}

/// Given two TICKCOUNT values, calculates microseconds between them.
fn tick_difference(start_time: Duration, end_time: Duration) -> u64 {
...
impl EMMCController {
    ...
    pub fn emmc_wait_for_interrupt(&self, mask: u32) -> SdResult {
        ...
        let mut start_time = Duration::new(0,0); // Zero start time
        ...
        while (self.registers.EMMC_INTERRUPT.get() & t_mask) == 0 && (time_diff < 1000000) {
            if start_time.as_nanos() == 0 {
                start_time = timer::current_time()
            }
            // If start time not set, then set start time
            else {
                time_diff = tick_difference(start_time, timer::current_time())
    ...
    pub fn emmc_wait_for_command(&self) -> SdResult {
        ...
        {
            if start_time.as_nanos()== 0 {
                start_time = timer::current_time(); // Get start time
            } else {
                time_diff = tick_difference(start_time, timer::current_time());
    ...
    pub fn emmc_set_clock2(&self, freq: u32) -> SdResult {
        ...
        // Timeout not reached
        {
            ...
            // If start time not set, then set start time
                ...
                td = tick_difference(start_time, timer::current_time());
            } // Time difference between start time and now
        }
